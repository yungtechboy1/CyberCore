 ...
 */
public class WalkingMonsterAsync extends AsyncTask {
    ...
    public WalkingMonsterAsync(WalkingMonster creeper, int Tickdiff, int CurrentTick) {
        CP = creeper;
        CT = CurrentTick;
        TD = Tickdiff;
    }
    ...
    public void onCompletion(Server server) {
        ...
        if (e instanceof WalkingMonster) {
            e.lastUpdate = CT;
            e.entityBaseTick(TD);
            WalkingMonster wm = (WalkingMonster) e;
            wm.checkTarget();
            if ((!wm.isFriendly() || !(wm.target instanceof Player)) && wm.target instanceof Entity) {
                if (wm.target != wm.followTarget || wm.canAttack) {
                    wm.attackEntity((Entity) wm.target);
                }

...
public abstract class BaseClass {
    ...
    public HashMap<Integer, PowerPublicInterface> Powers = new HashMap<>();
...
//        MainID = mid;
    ...
    public Collection<PowerPublicInterface> getPowers() {
        return Powers.values();
    }

    public PowerPublicInterface getPower(PowerEnum key) {
        return Powers.get(key.ordinal());
    }

    public abstract Object RunPower(PowerEnum powerid, Object... args);

    public void addPower(PowerPublicInterface power) {
...
//    }
    ...
    public boolean TryRunPower(PowerEnum powerid) {
        PowerPublicInterface p = getPower(powerid);
    ...
    public ArrayList<PowerPublicInterface> PossiblePowers() {
        ArrayList<PowerPublicInterface> a = new ArrayList<PowerPublicInterface>();
        return a;
    }
...
//        Event ee = e;
        for (PowerPublicInterface p : getPowers()) {
            p.handelEvent(e);
        }
        return e;
    }
...
//        }
    ...
    public CustomEntityDamageByEntityEvent CustomEntityDamageByEntityEvent(CustomEntityDamageByEntityEvent event) {
        for (PowerPublicInterface p : Powers.values()) p.CustomEntityDamageByEntityEvent(event);
...
//        System.out.println("TICKING BASECLASS");
    ...
    public String FormatHudText() {
        ...
        int plvl = lvl;
        f += TextFormat.AQUA + pclass + TextFormat.GRAY + " | " + TextFormat.GREEN + pxp + TextFormat.AQUA + " / " + TextFormat.GOLD + pxpof + TextFormat.GRAY + " | " + TextFormat.GREEN + "Level: " + TextFormat.YELLOW + plvl + TextFormat.GRAY+ " | " + TextFormat.AQUA+getPowerSourceType().name()+" PowerPublicInterface : "+getPowerSourceCount() + " / "+ getMaxPowerSourceCount();
        return f;
    }

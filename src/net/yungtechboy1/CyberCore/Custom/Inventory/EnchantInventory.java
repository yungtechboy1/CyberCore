...
public class EnchantInventory extends ContainerInventory {
    ...
    public EnchantInventory(Position position) {
        ...
        setContents(new HashMap<Integer,Item>(){{
            put(4,getItem(Item.GRASS));
        }});
        entries = new EnchantmentEntry[] {
            (new EnchantmentEntry( new Enchantment[] {new Spring()}, 1, "SSSSSSSSSSSSS")),
            (new EnchantmentEntry( new Enchantment[] {new Viper()}, 1, "VVVVVVVVV")),
            (new EnchantmentEntry( new Enchantment[] {new Restoration()}, 2, "RRRRRRRRRRR"))
        };
//        this.holder = new FakeBlockMenu(this, position);
    }
...
*/
    ...
    public void sendEnchantmentList(Player... players) {
        for(Player p : players){
            sendEnchantmentList(p);
        }
    }

    public void sendEnchantmentList(Player w) {
        w.sendMessage("SEND ENCHAT!!!!");
   ...
   @Override
    ...
    public boolean setItem(int index, Item item, boolean send) {
        System.out.println("SEEEEENNNDDD");
        sendEnchantmentList(getViewers().toArray(new Player[0]));
//            sendEnchantmentList(p);

...
public class Home extends Commands {
    ...
    public void RunCommand() {
        if(Args.length == 2){
            Faction ofaction = Main.FFactory.getFaction(Main.FFactory.factionPartialName(Args[1]));
            if(ofaction == null){
                Sender.sendMessage(FactionsMain.NAME+TextFormat.RED+"No Faction Found By That Name!");
                return;
            }
            if(!fac.isAllied(ofaction.GetName())){
            ...
            Sender.sendMessage(FactionsMain.NAME+TextFormat.GREEN+"Teleported to "+ofaction.GetName()+"'s home!");
            return;
        }else{
            if(fac == null){
            ...
            ((Player) Sender).teleport(home);
            Sender.sendMessage(FactionsMain.NAME+TextFormat.GREEN+"Teleported to faction home!");
            return;
        }
    }
}
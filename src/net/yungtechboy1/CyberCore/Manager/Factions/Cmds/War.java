...
//            }
            ...
            if(.5f > ratio){
                Sender.sendMessage(FactionsMain.NAME+TextFormat.RED+"Error, That target faction is not powerful enough for your faction to attack. The PowerAbstract Ration is "+ratio+" and it must be above .5 for you to attack!");
                return;
            }else if(ratio > 2f){
                Sender.sendMessage(FactionsMain.NAME+TextFormat.RED+"Error, That target faction is too powerful for your faction to attack. The PowerAbstract Ration is "+ratio+" and must be below 2 for you to attack!");
                return;
            }

            //ENEMY 40/100 FRIENDLY 50/70
            //      .40             .714
            //MIN 16 PowerAbstract for Attack
            // ROUND(50*(((40/100)+1)/2))
            // ROUND(50*(((.4)+1)/2))
            // ROUND(50*((1.4)/2))
            // ROUND(50*( .7 ))
            ...
            if (fac.GetPower() < takepower){
                Sender.sendMessage(FactionsMain.NAME+TextFormat.RED+"Error, Your faction does not have "+takepower+" PowerAbstract needed for this attack!");
                return;
            }
